...
A partir desta versão, produção deixa de depender de XAMPP. O padrão é **Ubuntu + Docker Engine + Docker Compose**. Cada módulo continua em seu próprio repositório e é incorporado ao stack como um serviço isolado.

## Serviços atuais

- `gateway` — Caddy, único ponto público do stack (`80/443`) e TLS automático.
- `hub-core` — núcleo PHP do HUB, preparado para MySQL.
- `mysql` — banco central do HUB e base para novos módulos.
- `totem-api` — backend do `totem_autoatendimento`, Node.js 20, com dados persistidos em volume.
...
Nenhuma aplicação publica `3080`, `8091` ou `3306` no host.
...
- rollback é feito por versão de imagem/commit, não por edição manual no servidor.

## XAMPP

XAMPP pode permanecer temporariamente em estações antigas de desenvolvimento, mas **não faz parte da arquitetura de produção**. O objetivo é que desenvolvimento e produção utilizem o mesmo Compose.